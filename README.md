...
O **transk table** transcreve uma tabela por vez. Para iniciar a transcrição o usuário deve definir o número da página do documento onde está a tabela de interesse e clicar em *open* para abrir o documento. Após a seleção do documento, o **transk table** vai criar uma pasta cujo nome será dado pelo número da página. Dentro dessa pasta será salvo um arquivo em csv chamado "DDD_page.csv", onde DDD é o número da página. O arquivo "DDD_page.csv" é a transcrição bruta feita pelo tabular. Dessa forma, a simples definição da página e abertura documento encerra o passo 1.
